...
function returnable(ret) {
  ...
  if (ptr >= 1) return "iptr";                    // pointer return → opaque int handle (cast (int) at call)
...
function generate() {
  ...
  const lines = rows.map((r) => {
    ...
    // a pointer return ("iptr") is an int handle to the type system, but the
    // call must cast the Sprite*/Map*/... to int (retptr: true) so assigning it
    // to an int global doesn't trip -Wint-conversion under -Werror.
    const isPtr = r.ret === "iptr";
    const ret = isPtr ? "int" : r.ret;
    const rp = isPtr ? ", retptr: true" : "";
    return `  ${r.name}: { params: [${ps}], ret: "${ret}", c: "${r.name}", sgdk: true${rp} },`;
  });
  const body =
